...
import { Query, items, collection } from '@directus/types'
...
const { item } = useItems(collection, primaryKey, query)
  ...
  for (const field of fields.value) {
    if (
      field.meta?.special?.some((special) =>
        RELATIONAL_TYPES.includes(special as (typeof RELATIONAL_TYPES)[number]),
      )
    ) {
      continue
    }
